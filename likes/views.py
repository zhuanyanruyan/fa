...
def like_change(request):
    ...
    content_type = request.GET.get('content_type')  # 这个还是字符串形式的
    object_id = int(request.GET.get('object_id'))

    try:
        content_type = ContentType.objects.get(model=content_type)  # 转成对象
        model_class = content_type.model_class()  # 得到对应的类
